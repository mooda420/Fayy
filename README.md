...
- **Rider shift tab**: 18 seeded deliveries (6 pickups → 12 drop-offs chosen by `pipeline/fleet.py` from a seeded pool of 300 street points as the trips with the biggest shade contrast, afternoon 15:30–18:30 by default with a Midday 11:00–15:00 toggle, same seed, Motorcycle, Balanced; schedule from `pipeline/fleet.py` → `docs/data/fleet.json`), routed live in the browser, with a per-delivery bar chart of sun minutes, time / km / sun totals for shortest vs Fayy, sun minutes avoided per extra km, and an extra-fuel estimate (3.0 L/100 km, AED 2.8/L). At midday about 90% of streets are in direct sun, so the saving is small (about 1% on 25 Sep); the tab says so.
